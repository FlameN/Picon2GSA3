...
void MakeiFixUIP()
    ...
    RAM.UIPparams[9] = ((uint32)(atof(&RAM.strings[4][0])*65536/40))&0xFF;
    RAM.UIPparams[10] = (((uint32)(atof(&RAM.strings[4][0])*65536/40))>>8)&0xFF;
/*
...
void vRs485Task(void *pvParameters)
    ...
    while (1)
	{
	AT91F_PIO_ClearOutput(AT91C_BASE_PIOB, USART2_RTS);

	rs485size = AT91F_USART_RECEIVE(&COM2,rs485buf,256,5000);
	if(rs485size!=0)
	    {

	    AT91F_PIO_SetOutput(AT91C_BASE_PIOB, LED_485);

	    rs485size = RVnetSlaveProcess(rs485buf, rs485size,1);
	    AT91F_PIO_SetOutput(AT91C_BASE_PIOB, USART2_RTS);

	    vTaskDelay(5);
	    AT91F_USART_SEND(&COM2,rs485buf,rs485size);

	    AT91F_PIO_ClearOutput(AT91C_BASE_PIOB, LED_485);
	    }
	vTaskDelay(1);
	}
#else
